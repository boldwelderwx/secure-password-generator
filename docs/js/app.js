...
async function loadPresets() {
  try {
    const r = await fetch(WORKER_URL + '/presets');
    ...
    if (g) g.innerHTML = '<p style="color:#ef4444;">Failed to load presets. Check console.</p>';
  }
}
...
function renderPresets(presets) {
  ...
  f.innerHTML = '<button class="category-btn active" data-cat="all">All</button>' +
    cats.map(c => '<button class="category-btn" data-cat="' + c + '">' + c + '</button>').join('');
  
  f.querySelectorAll('.category-btn').forEach(b => {
  ...
  g.innerHTML = presets.map(p => 
    '<div class="preset-card" data-cat="' + p.category + '" onclick="copyPreset(\'' + p.name + '\')">' +
    '<span class="preset-category">' + p.category + '</span>' +
    '<h4>' + p.name + '</h4>' +
    '<p>' + p.description + '</p>' +
    '<div class="preset-meta"><span>📏 ' + p.length + ' chars</span><span>📊 ' + p.count.toLocaleString() + '</span></div>' +
    '</div>'
  ).join('');
}

function copyPreset(name) {
  const cmd = 'curl --compressed ' + WORKER_URL + '/preset/' + name + ' -o pw.csv';
...
function showToast(msg) {
  ...
  toast.style.cssText = 'position:fixed;bottom:2rem;left:50%;transform:translateX(-50%);background:#10b981;color:white;padding:0.75rem 1.5rem;border-radius:0.5rem;z-index:9999;font-weight:600;';
...
function renderBoldwelder(d) {
  ...
  el.innerHTML = 
    ...
    d.openssl_passfile_examples.map(ex => 
      '<div class="workflow-card"><h4>' + ex.tool + '</h4>' +
...
function renderSarahTim(d) {
  ...
  el.innerHTML = 
    ...
    d.full_workflow.map(s => 
      '<div class="workflow-card"><h4>Step ' + s.step + ': ' + (s['title_' + lang] || s.title_en) + '</h4>' +
    ...
    d.openssl_algorithms.map(a => 
      '<tr style="border-bottom:1px solid var(--brd);"><td style="padding:.75rem;"><code>' + a.name + '</code></td><td>' + a.security + '</td><td>' + a.use + '</td><td>' + a.note + '</td></tr>'
    ).join('') + '</table>';
}
...
function renderKyber(d) {
  ...
  el.innerHTML = 
    ...
    d.kyber_variants.map(v => 
      '<tr style="border-bottom:1px solid var(--brd);"><td style="padding:.75rem;"><code>' + v.name + '</code></td><td>' + v.security + '</td><td>' + v.use + '</td><td>' + v.pubkey + '</td><td>' + v.privkey + '</td><td>' + v.ciphertext + '</td></tr>'
    ).join('') + '</table>' +
...
function renderEmailProviders(d) {
  ...
  el.innerHTML = 
    ...
    d.providers.map(p => 
      '<div class="email-card"><h4>' + p.name + '</h4>' +
      '<div class="email-meta"><span>Max: <span class="email-max">' + p.max + ' chars</span></span><span>Recommended: ' + p.recommended + '</span></div>' +
      '<p style="font-size:.75rem;color:var(--txt2);margin-top:.5rem;">' + p.notes + '</p></div>'
    ).join('') + '</div>';
}
...
function renderAnonymity(d) {
  ...
  el.innerHTML = 
    ...
    d.levels.map(l => 
      '<div class="workflow-card"><h4>Level ' + l.level + ': ' + l.name + ' (' + l.security + ')</h4>' +
...
function setupTabs() {
  document.querySelectorAll('.tab').forEach(t => {
    t.addEventListener('click', function() {
      const tgt = this.dataset.tab;
      document.querySelectorAll('.tab').forEach(x => x.classList.remove('active'));
      document.querySelectorAll('.tab-content').forEach(c => c.classList.remove('active'));
      this.classList.add('active');
      const target = document.getElementById('tab-' + tgt);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  loadJSON('content/tor-guide.json', function(d) {
    renderAnonymity(d);
    renderTextBrowsers(d);
  });
  setupTabs();
